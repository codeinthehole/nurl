...
Url.prototype.setPathSegment = function(index, newSegment) {
	...
	if (index < pathSegments.length) {
		...
		pathSegments.push(escape(newSegment));
	} else {
		throw new Error("Invalid path index: "+index);
	}
	return createFromProperties(this.__setProperty('pathname', '/'+pathSegments.join('/')));
};
Url.prototype.setPathSegments = function(segments) {
	var escapedSegments = [];
	for (key in segments) {
		escapedSegments.push(escape(segments[key]));
	}
	return this.setPathname("/"+escapedSegments.join("/"));
};
...
exports.createFromString = function(urlString) {
    ...
    var hostname = libUrl.hostname ? libUrl.hostname : null;

    var pathname = libUrl.pathname ? libUrl.pathname : null;
    ...
    var hash = libUrl.hash ? libUrl.hash.substring(1) : null;
